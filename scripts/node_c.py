...
class InfoService:
    def __init__(self):
        ...
        # Provide a service named 'info_service', using the custom service type average
        rospy.Service("info_service", average, self.get_values)
        # Subscribe to the '/position_vel' topic, using the custom message type Vel
        rospy.Subscriber("/position_vel", Vel, self.get_distance_and_averagevelocity)

    # Callback function for the subscriber
    def get_distance_and_averagevelocity(self, msg):
        # Get the desired x and y positions from the parameter server
        des_x = rospy.get_param('/des_position_x')
        des_y = rospy.get_param('/des_position_y')
        ...
        actual_x = msg.position_x
        actual_y = msg.position_y
    ...
    def get_values(self, _):      
        ...
        return averageResponse(self.distance, self.average_vel_x)		      
...
if __name__ == "__main__":
    ...
    service = InfoService()
    dist_vel_service = rospy.ServiceProxy('info_service', average)
